...
test('custom image field needs specific format', async () => {
  ...
  expect(screen.getByText('Must be a publicly available docker image, of form <image-name>:<tag>')).toBeInTheDocument();
});
...
test('custom image field accepts specific format', async () => {
  ...
  expect(screen.queryByText('Must be a publicly available docker image, of form <image-name>:<tag>')).not.toBeInTheDocument();
});
